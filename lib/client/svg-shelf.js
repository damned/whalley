var whalley = whalley || {};
whalley.SvgShelf = function (wall_view_api, cards_api, added_event) {
  var shelf, shelf_y;
  var blanks;

  function create_with_view(cardlike) {
    var logical = new whalley.LogicalCard(cardlike, wall_view_api)
    wall_view_api.create_card_view(logical)
    return logical
  }

  function create_blank_cards() {
    var blanks = []
    blanks.push(create_with_view({ id: 'blank_x', width: 50, height: 60, color: 'white', text: wall_view_api.DEFAULT_TEXT }))
    blanks.push(create_with_view({ id: 'blank_xx', width: 100, height: 60, color: 'white', text: wall_view_api.DEFAULT_TEXT }))
    return blanks
  }
  function init_shelf(blanks, y) {
    ...
    var shelf = wall_view_api.group(400, 100).move(20, y)
