var whalley = whalley || {};
whalley.LogicalWall = function (physical) {
  var self = this;
  var event = whalley.event;
  var background_image, width, height;

  self.events = {
    card_moving: event(),
    card_changed: event(),
    card_added: event()
  };

  self.set_background_image = function(image) {
    background_image = image
    physical.set_background_image(background_image);
  }

  function set_size(w, h) {
    width = w
    height = h
    physical.set_size(width, height)
  }

  self.wall_data = function(data) {
    if (data) {
      if (data.background_image) {
        self.set_background_image(data.background_image)
      }
      if (data.width && data.height) {
        set_size(data.width, data.height);
      }
    }
    else {
      return {
        background_image: background_image,
        width: width,
        height: height
      }
    }
  }

  self.move_to_front = function(id, cards) {
    cards.some(function (a_card, a_card_index) {
      if (a_card.has_id(id)) {
        cards.splice(a_card_index, 1)
        cards.push(a_card)
        return true;
      }
    });
    return cards;
  }
}