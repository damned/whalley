...
whalley.SvgCardView = function(wall, logical) {
  ...
  function create_menu() {
    const radius = 20
    var menu_button = group.circle(2 * radius).dmove(-radius, -radius).front().fill('url(#menu_gradient)')
    return whalley.SvgMenu(menu_button, wall, [
          {
            name: 'option a',
            action: function() {
              alert('optione AAAA')
  ...
  function clicked(subject, target) {
    if (card_menu.handle(subject, target)) {
      return
    }
    console.log('editing...')
