...
whalley.LogicalCard = function (cardlike, wall) {
  ...
  add_get_fn('x', function() { return data.x })
  add_get_fn('y', function() {
    return data.y
  })
  ...
  self.move_completed = function() {
    if (on_moved_handler) on_moved_handler()
  }
  var on_moved_handler;
  self.on_moved = function(handler) {
    on_moved_handler = handler
  }
