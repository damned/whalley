...
whalley.LogicalCard = function (cardlike, wall) {
  ...
  self.update_to = function(new_data) {
    if (data.text !== new_data.text) {
      var old_text = data.text;
      data.text = new_data.text;
      card_view.update_text();
      console.log("updated card text from, to:", old_text, new_data.text);
    }
    else {
      console.warn("got update_to request but not handled what's being updated :/", data, new_data);
    }
  }
