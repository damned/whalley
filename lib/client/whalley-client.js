...
whalley.FabricWall = function() {
  ...
  return {
    ...
    move_card: move_card,
    on_card_move: events.card_move.listen
  }
}
...
var load_wall = function(ui) {
  ...
  fetch_mingle_cards(username, password, function(card_data) {
    visible_wall.set_cards(card_data);
...
whalley.SocketIoSyncClient = function(wall) {
  ...
  wall.on_card_move(external.card_moved)

  socket.on('card.move', function(card_data) {
    wall.move_card(card_data);
  });

  socket.on('wall.reload', function(cards_data) {
    wall.set_cards(cards_data);
  });
