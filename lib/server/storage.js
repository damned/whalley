...
function convert_wall_json(data, version) {
  ...
  return JSON.stringify(data_converter.convert(wall_data, version));
}
...
function write_wall_data(request, response, options) {
  ...
  if (request.method === options.method) {
    ...
    request.on('end', function() {
      ...
      var converted = convert_wall_json(data, options.version);
      if (read_data(resource_path) != converted) {
        util.log('saving data: ' + converted.substring(0, 20) + '(...)');
        write_data(resource_path, converted);
      }
