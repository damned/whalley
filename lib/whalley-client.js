...
function store_to_cookie (id, key, value) {
  $.cookie('' + id + '-' + key, value);
}

function read_from_cookie (id, key) {
...
var save_cards = function() {
  ...
  if (local_card_ids.length > 0) {
    ...
    $.cookie('local-card-ids', local_card_ids);
  }
}

var restore_cards = function() {
  cards.forEach(function(card) {
    card.restore(read_from_cookie);
  });
  canvas.renderAll();
}
...
var load_local_cards = function() {
  ...
  var local_card_ids = $.cookie('local-card-ids');
