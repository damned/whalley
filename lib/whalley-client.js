...
canvas.observe('object:selected', function (event) {
  if (event.target !== undefined) {
    ...
    event.target.get('card').reangle();
  }
});

canvas.observe('selection:created', function (event) {
   lockdown(event.target);
});
...
function Card(params) {
  ...
  this.left = function(new_value) {
    if (new_value === undefined) {
      return card_group.getLeft();
    }
    card_group.setLeft(new_value);
  };
  
  this.top = function(new_value) {
    if (new_value === undefined) {
      return card_group.getTop();
    }
    card_group.setTop(new_value);
  };

  this.angle = function(new_value) {
    if (new_value === undefined) {
      return card_group.getAngle();
    }
    card_group.setAngle(new_value);
  };
